...
public final class MyndSDK {
  ...
  public init(
    ...
    let listeningSessionManager = ListeningSessionManager()
    let eventTrackingClient = EventTrackingClientInfraService(
      config: .init(authedHttpClient: authedHttpClient))

    self.player = AudioClient(
      configuration: audioConfiguration, listeningSessionManager: listeningSessionManager,
